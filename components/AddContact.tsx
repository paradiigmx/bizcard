...
import type { Contact, Event, EventRole, EventLink, ContactType } from '../types';
...
const AddContact: React.FC<AddContactProps> = ({ onSaveContact, onCancel, allTags, events, defaultEventId, onCreateEvent, prefillData, existingContacts }) => {
    const getInitialState = () => ({
        name: '', role: '', company: '', email: '', phone: '', websites: [],
        address: { street: '', city: '', state: '', postal_code: '', country: 'United States' },
        notes: '', tags: [], image_url: undefined, follow_up_date: undefined,
        eventLinks: defaultEventId ? [{ eventId: defaultEventId, role: 'Attendee' as EventRole }] : [], 
        isFavorite: false,
        contactType: 'Lead' as ContactType,
    });
    
    const [contact, setContact] = useState<Omit<Contact, 'id'>>(prefillData || getInitialState());
    
    useEffect(() => {
        if (prefillData) {
            setContact(prefillData);
        } else {
             // Automatically open camera if not prefilling data
            openCamera();
        }
    }, [prefillData]);
    ...
    const handleReminderChange = (date: string | null) => {
        setContact(prev => ({...prev, follow_up_date: date || undefined}));
    };
    ...
    const handleEventLinkChange = (index: number, field: keyof EventLink, value: string) => {
        ...
        setContact(prev => ({ ...prev, eventLinks: newEventLinks }));
    };
    ...
    const handleWebsiteChange = (index: number, value: string) => {
        ...
        setContact(prev => ({ ...prev, websites: newWebsites }));
    };

    const addWebsite = () => {
        setContact(prev => ({ ...prev, websites: [...(contact.websites || []), ''] }));
    };

    const removeWebsite = (index: number) => {
        setContact(prev => ({ ...prev, websites: (contact.websites || []).filter((_, i) => i !== index) }));
    };
    ...
    const processFile = (file: File) => {
        ...
        reader.onloadend = () => {
            ...
            setContact(prev => ({...prev, image_url: dataUrl}));
            parseImage(file);
        };
        ...
        <>
            {isCameraOpen && (
                <div className="fixed inset-0 bg-black z-50 flex flex-col items-center justify-center p-4">
                    ...
                    <div className="absolute top-4 right-4 flex flex-col gap-3">
                        <button onClick={onCancel} className="text-white bg-black bg-opacity-50 rounded-full p-2">
            ...
            <div className="p-4 sm:p-6 bg-[rgb(var(--color-bg-secondary))] rounded-lg shadow space-y-6">
                ...
                {isProcessing ? <SkeletonLoader /> : (
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <CustomSelect options={CONTACT_TYPES} value={contact.contactType} onChange={(v) => setContact(p => ({...p, contactType: v as ContactType}))} />
                            ...
                            <input name="company" value={contact.company} onChange={handleChange} placeholder="Company" className="w-full bg-[rgb(var(--color-bg-tertiary))] p-2 rounded" />
                        ...
                        <div className="space-y-2 p-3 border border-[rgb(var(--color-border))] rounded-md">
                            ...
                            <input name="address.street" value={contact.address.street} onChange={handleChange} placeholder="Street" className="w-full bg-[rgb(var(--color-bg-tertiary))] p-2 rounded" />
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><input name="address.city" value={contact.address.city} onChange={handleChange} placeholder="City" className="w-full bg-[rgb(var(--color-bg-tertiary))] p-2 rounded" /><input name="address.postal_code" value={contact.address.postal_code} onChange={handleChange} placeholder="Postal Code" className="w-full bg-[rgb(var(--color-bg-tertiary))] p-2 rounded" /></div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><select name="address.country" value={contact.address.country} onChange={(e) => handleChange(e as any)} className="w-full bg-[rgb(var(--color-bg-tertiary))] p-2 rounded">{countries.map(c => <option key={c} value={c}>{c}</option>)}</select><select name="address.state" value={contact.address.state} onChange={(e) => handleChange(e as any)} className="w-full bg-[rgb(var(--color-bg-tertiary))] p-2 rounded" disabled={states.length === 0}><option value="">-- Select State --</option>{states.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                        </div>
                ...
                <div className="flex justify-end gap-3 pt-4">
                    <button onClick={onCancel} className="px-4 py-2 font-semibold bg-[rgb(var(--color-bg-subtle))] text-[rgb(var(--color-text-primary))] rounded-md hover:opacity-80">Cancel</button>
                    <button onClick={handleSave} className="px-4 py-2 font-semibold bg-[rgb(var(--color-primary))] text-[rgb(var(--color-primary-text))] rounded-md hover:bg-[rgb(var(--color-primary-hover))] disabled:opacity-50" disabled={!contact.name || isProcessing}>{isProcessing ? 'Processing...' : 'Save Contact'}</button>
                </div>
            </div>
        </>
    );
