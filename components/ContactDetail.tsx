...
const ContactDetail: React.FC<ContactDetailProps> = ({ contact, onBack, onUpdateContact, onDelete, onToggleFavorite, allTags, events, onCreateEvent }) => {
    ...
    const handleShare = async () => {
        ...
        try {
            ...
            if (!shareData.files) {
                alert('Sharing failed. Please try downloading the contact card instead.');
            } else {
                 handleDownloadVCF();
            }
        }
            ...
            <div className="p-4 sm:p-6 bg-[rgb(var(--color-bg-secondary))] rounded-lg shadow space-y-4">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {!isParadiigmContact && (
                            <button onClick={handleDelete} className="p-2 text-[rgb(var(--color-danger))] hover:opacity-80">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2 p-3 border border-[rgb(var(--color-border))] rounded-md">
                         ...
                         <div className="space-y-2 p-3 border border-[rgb(var(--color-border))] rounded-md">
                            ...
                            <input name="address.street" value={editedContact.address.street} onChange={handleChange} placeholder="Street" className="w-full bg-[rgb(var(--color-bg-tertiary))] p-2 rounded" />
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <input name="address.city" value={editedContact.address.city} onChange={handleChange} placeholder="City" className="w-full bg-[rgb(var(--color-bg-tertiary))] p-2 rounded" />
                                <input name="address.postal_code" value={editedContact.address.postal_code} onChange={handleChange} placeholder="Postal Code" className="w-full bg-[rgb(var(--color-bg-tertiary))] p-2 rounded" />
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                 <select name="address.country" value={editedContact.address.country} onChange={(e) => handleChange(e as any)} className="w-full bg-[rgb(var(--color-bg-tertiary))] p-2 rounded">
                                    {countries.map(c => <option key={c} value={c}>{c}</option>)}
                                </select>
                                <select name="address.state" value={editedContact.address.state} onChange={(e) => handleChange(e as any)} className="w-full bg-[rgb(var(--color-bg-tertiary))] p-2 rounded" disabled={states.length === 0}>
                    ...
                    <div className="space-y-3">
                        ...
                        <div>
                            ...
                            <p className="text-lg text-[rgb(var(--color-text-secondary))]">{contact.role} at {contact.company}</p>
                             <span className="mt-1 inline-flex items-center gap-1.5 px-2 py-0.5 font-medium bg-teal-500/10 text-teal-800 dark:text-teal-300 rounded-md">
